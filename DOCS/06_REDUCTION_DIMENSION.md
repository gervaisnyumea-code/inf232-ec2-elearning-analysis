# INF 232 EC2 — RÉDUCTION DE DIMENSION
## ACP · t-SNE · LDA — Théorie, Implémentation et Diagrammes

---

## 1. CADRE THÉORIQUE

### 1.1 ACP — Analyse en Composantes Principales

**Nature :** Méthode linéaire, non supervisée  
**Objectif :** Projeter les données dans un espace de dimension réduite en maximisant la variance expliquée

**Algorithme :**
1. Centrer-réduire les données : `X_std = (X - μ) / σ`
2. Calculer la matrice de covariance : `Σ = XᵀX / (n-1)`
3. Décomposition en valeurs propres : `Σvᵢ = λᵢvᵢ`
4. Trier par λᵢ décroissant
5. Projeter : `Z = X_std · V_k` (k premières composantes)
...
**Règle de Kaiser :** Retenir les composantes avec λ > 1

---

### 1.2 t-SNE — t-Distributed Stochastic Neighbor Embedding

**Nature :** Méthode non linéaire, non supervisée  
**Objectif :** Visualisation 2D/3D de données haute dimension en préservant les structures locales

**Principe :**
- En haute dimension : modélise la similarité entre points par une distribution gaussienne
- En basse dimension : modélise les similarités par une distribution de Student-t (queue plus lourde)
- Minimise la divergence KL entre les deux distributions : `KL(P || Q) = Σ pᵢⱼ log(pᵢⱼ/qᵢⱼ)`

**Paramètre clé :** `perplexity` (~5-50), contrôle l'équilibre local/global

---

### 1.3 LDA — Analyse Discriminante Linéaire

**Nature :** Méthode linéaire, **supervisée**  
**Objectif :** Trouver les projections qui maximisent la séparabilité entre classes
...
- `SW` = matrice de dispersion intra-classes (Within-class scatter)
- `w` = vecteur de projection optimal

**Nombre d'axes discriminants :** min(K-1, p) où K = nb classes, p = nb features

---

## 2. MODULE `src/dimension_reduction.py`
...
class AnalyseACP:
    ...
    def fit_transform(
        ...
        feature_names : List[str]
            Noms des features originales.
        
        Returns
        -------
        np.ndarray
            Coordonnées dans l'espace ACP réduit.
        """
    ...
    def plot_biplot(
        ...
        Parameters
        ----------
        labels : array optionnel
            Labels pour colorier les individus (clusters ou classes).
        label_type : str
            'cluster' ou 'class'
        scale : float
            Facteur d'échelle pour les vecteurs de chargement.
        """
...
class VisualisationTSNE:
    ...
    def fit_transform(self, X_scaled: np.ndarray) -> np.ndarray:
        ...
        Notes
        -----
        t-SNE est stochastique. Les positions absolues des clusters
        ne sont pas interprétables, seule la structure locale l'est.
        """
...
def run_dimension_reduction_pipeline(
    ...
    -------
    dict
        Contient les objets ACP, t-SNE, LDA et les coordonnées.
    """
...
## 3. TABLEAU COMPARATIF ACP vs t-SNE vs LDA

| Critère | ACP | t-SNE | LDA |
|---------|-----|-------|-----|
...
### ACP
```
PC1 : 38.2% de variance expliquée
PC2 : 22.5% de variance expliquée
PC1+PC2 : 60.7% de variance expliquée cumulée
Composantes pour 95% de variance : 6

Variables les plus importantes pour PC1 :
  temps_etude_hebdo    (chargement : +0.45)
  nb_devoirs_rendus    (chargement : +0.42)
  exercices_pct        (chargement : +0.38)
  nombre_absences      (chargement : -0.31)
```

### LDA (1 axe discriminant pour K=2)
```
Variable la plus discriminante : temps_etude_hebdo (|coef| = 0.412)
Suivi de : nb_devoirs_rendus (0.389), exercices_pct (0.351)

Exactitude LDA (leave-one-out) : ~82%
```

---

*Document 7/10 — INF 232 EC2*
